/**
 * OFAC-designated Ethereum addresses.
 *
 * Source: 0xB10C/ofac-sanctioned-digital-currency-addresses, the community
 * mirror that parses Treasury's SDN list into per-chain files. It's the list
 * Ethereum block builders use for OFAC filtering, so it's well-exercised — but
 * it is a third-party mirror, not Treasury directly. For anything real, verify
 * against https://sanctionssearch.ofac.treas.gov or use a live oracle.
 *
 * THIS IS A SNAPSHOT. It was correct when written and drifts from then on —
 * a designation added tomorrow will be approved by this policy. That staleness
 * is the entire reason ../sanctions-oracle/ exists: it reads the Chainalysis
 * on-chain oracle at evaluation time, so it can't go stale.
 *
 * Refresh with:
 *   curl -s https://raw.githubusercontent.com/0xB10C/ofac-sanctioned-digital-currency-addresses/lists/sanctioned_addresses_ETH.json
 */

export const OFAC_ETH_ADDRESSES: string[] = [
...
/** Snapshot date, so the UI can say how stale it is. */
export const OFAC_SNAPSHOT_DATE = "2026-08-13";
